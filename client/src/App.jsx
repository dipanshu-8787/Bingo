...
import {  FaClock, FaVolumeUp, FaVolumeMute } from "react-icons/fa";
...
function App() {
  ...
  // Use refs for timer and audio to avoid closure issues
  ...
  useEffect(() => {
    ...
    // Load and initialize audio with correct paths for deployment
    ...
    newSocket.on('gameInvite', (data) => {
      if (soundEnabled) inviteSoundRef.current.play().catch(err => console.log('Audio play failed:', err));
    ...
    newSocket.on('updateMatrix', (data) => {
      ...
      if (soundEnabled) {
        if (data.lineCompleted) {
          ...
          turnSoundRef.current.play().catch(err => console.log('Audio play failed:', err));
        }
      }
    });
    ...
    newSocket.on('gameOver', (data) => {
      ...
      if (soundEnabled) winSoundRef.current.play().catch(err => console.log('Audio play failed:', err));
    ...
    newSocket.on('turnChange', (playerId) => {
      ...
      if (soundEnabled) turnSoundRef.current.play().catch(err => console.log('Audio play failed:', err));
    });
